...
def get_source_query_map(table_name, sub_query, row_key, validation_type):
    if validation_type == "count_validation":
        query = f"SELECT COUNT(*) AS ROWCOUNT FROM {table_name}"
        if sub_query:
            query += sub_query
    elif validation_type == "number_validation":
        query = f"SELECT SUM({row_key}) AS NUMBERCOUNT FROM {table_name}"
        if sub_query:
            query += sub_query
    elif validation_type == "string_validation":
        query = f"SELECT SUM(LENGTH({row_key}) - LENGTH(REPLACE({row_key}, ' ', '')) + 1) AS WORDCOUNT FROM {table_name}"
        if sub_query:
            query += sub_query
    return query
